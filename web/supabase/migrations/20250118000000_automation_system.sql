...
CREATE TABLE IF NOT EXISTS user_listening_stats (
    ...
    listening_patterns JSONB DEFAULT '{}'::jsonb, -- time of day, day of week patterns
...
BEGIN
    ...
    -- Calculate listening patterns (hour of day, day of week)
    SELECT jsonb_build_object(
...
-- Function to generate Daily Mix for a user
...
BEGIN
    ...
    FROM (
        ...
        -- 40% from similar artists/genres
        SELECT DISTINCT t.id as track_id
        FROM tracks t
...
BEGIN
    ...
    FROM (
        ...
        AND t.created_at > NOW() - INTERVAL '90 days' -- Recent tracks
...
BEGIN
    ...
    FROM (
        ...
        AND t.created_at > NOW() - INTERVAL '30 days' -- Last 30 days
...
BEGIN
    ...
    FROM (
        ...
        HAVING COUNT(*) >= 3 -- Played at least 3 times
        ORDER BY COUNT(*) DESC
        LIMIT 30
    ) forgotten_tracks;
...
BEGIN
    ...
    SELECT
        track_id,
        -- Trend score formula: weighted by recency and uniqueness
        (
            (play_count_24h * 10.0) +
            (play_count_7d * 2.0) +
    ...
    FROM (
        ...
        GROUP BY ph.track_id
        HAVING COUNT(*) >= 3 -- Minimum plays to be considered trending
    ) trending_data
...
CREATE TABLE IF NOT EXISTS background_tasks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    task_type TEXT NOT NULL, -- 'mood_detection', 'energy_analysis', 'genre_classification'
...
AS $$
BEGIN
    -- Queue mood detection task for new track
