...
export const MobilePlayer = ({ isExpanded, onExpand, onCollapse }: MobilePlayerProps) => {
  ...
  const dragOffset = isDragging ? Math.max(0, touchCurrentY - touchStartY) : 0;
  ...
  if (!isExpanded) {
    return (
      <div
        onClick={onExpand}
        className="fixed bottom-16 left-0 right-0 bg-gradient-to-r from-gray-900 to-black border-t border-white/10 z-50 cursor-pointer active:scale-[0.99] transition-transform duration-150"
      >
        {/* Progress bar at top */}
  ...
  return (
    ...
    >
      <div
        className="h-full bg-gradient-to-b from-gray-900 via-black to-black flex flex-col overflow-hidden"
      ...
      >
        ...
        <div className="flex-shrink-0">
          {/* Swipe down indicator - tappable to collapse */}
          <button
            onClick={onCollapse}
            className="w-full flex justify-center pt-3 pb-2 active:bg-white/5"
          >
          ...
          <div className="flex items-center justify-between px-4 pb-2">
            ...
            <div className="text-sm font-medium text-white/70">Now Playing</div>
            ...
            <div className="relative" ref={menuRef}>
              ...
              {/* Dropdown Menu */}
              {showMenu && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-gray-800 rounded-xl shadow-2xl border border-white/10 overflow-hidden z-50">
                  <button
                    onClick={() => {
                      if (navigator.share) {
                        navigator.share({
        ...
        <div className="flex-1 flex items-center justify-center px-8 py-4 min-h-0">
          <div className="relative w-full max-w-[320px] aspect-square rounded-xl overflow-hidden shadow-2xl">
            {currentTrack.image ? (
              <Image
                src={currentTrack.image}
                alt={currentTrack.title}
        ...
        <div className="px-8 pb-6 flex-shrink-0">
          <div className="flex items-center justify-between max-w-sm mx-auto">
            ...
            <button
              ...
              className={`transition-colors active:scale-90 ${shuffle ? 'text-primary' : 'text-gray-400'}`}
            >
            ...
            {/* Previous */}
            <button onClick={skipBackward} className="text-white active:scale-90 transition-transform">
            ...
            {/* Next */}
            <button onClick={skipForward} className="text-white active:scale-90 transition-transform">
              <svg className="w-10 h-10" fill="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              className={`transition-colors active:scale-90 relative ${repeat !== 'off' ? 'text-primary' : 'text-gray-400'}`}
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        {showQueue && (
          <div className="absolute inset-0 bg-black z-10">
            ...
            <div className="overflow-y-auto px-4 pb-32 h-[calc(100%-64px)]">
              {queue.length === 0 ? (
